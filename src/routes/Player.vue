<template>
  <div id="container" ref="container">
    <div
      id="player-wrapper"
      @click="
        segmentCutExpanded = false;
        playbackSpeedExpanded = false;
      "
    >
      <span id="loading">Loading...</span>
      <div id="top-buttons">
        <button @click="router.go(-1)" class="material-symbols-outlined ui-btn">
          arrow_back
        </button>
        <button
      ...
      <div id="toolbar">
        <div id="sliders" @click="$event.stopPropagation()">
          <Slider
            v-if="playbackSpeedExpanded"
            v-model="playbackSpeed"
            class="no-connect"
            :min="0.5"
            :max="1.5"
            :step="0.01"
            :format="{ decimals: 1, suffix: 'x' }"
          />
          <Slider
            v-if="segmentCutExpanded"
            v-model="segmentAbsolute"
            :min="0"
            :max="vidLength"
            :step="0.01"
            :format="{ decimals: 1, suffix: 's' }"
          />
        </div>
        <div id="buttons">
          <div class="toolbar-item">
            <button
              id="mirror-button"
              class="material-symbols-outlined ui-btn"
              :class="{ active: mirrorVid }"
              @click="mirrorVid = !mirrorVid"
            >
              flip
            </button>
          </div>
          <div class="toolbar-item" @click="$event.stopPropagation()">
            <button
              ...
              @click="
                playbackSpeedExpanded = !playbackSpeedExpanded;
                segmentCutExpanded = false;
              "
            >
              speed
          ...
          <div class="toolbar-item" @click="$event.stopPropagation()">
            <button
              ...
              @click="
                segmentCutExpanded = !segmentCutExpanded;
                playbackSpeedExpanded = false;
              "
            >
              cut
      ...
      <video
        ...
        :src="route.params.url + '#t=0.1'"
        :class="{ mirrored: mirrorVid }"
        @loadedmetadata="vidLength = $event.target.duration"
        loop
...
const toggleFullscreen = () => {
  if (!isFullscreen.value) requestFullscreen(container.value);
  else exitFullscreen();
